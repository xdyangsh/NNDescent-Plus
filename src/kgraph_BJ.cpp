...
#pragma omp critical
    ...
    class KGraphConstructor : public KGraphImpl
    {
        ...
        { // neighborhood
            ...
            template <typename C>
            void join(C callback, unsigned numInOneBlock) const
            {
                unsigned nn_new_size = nn_new.size();
                unsigned nn_old_size = nn_old.size();
                unsigned block_new = ceil(nn_new_size / float(numInOneBlock));
                unsigned block_old = ceil(nn_old_size / float(numInOneBlock));
                for (unsigned k = 0; k < block_new; k++)
                {
                    for (unsigned l = k; l < block_new; l++)
                    {
                        if (k == l)
                        {
                            for (unsigned i = l * numInOneBlock; i < nn_new_size && i < (l + 1) * numInOneBlock; i++)
                            {
                                for (unsigned j = i + 1; j < nn_new_size && j < (k + 1) * numInOneBlock; j++)
                                {
                        ...
                        else
                        {
                            ...
                            for (unsigned i = l * numInOneBlock; i < nn_new_size && i < (l + 1) * numInOneBlock; i++)
                            {
                                for (unsigned j = k * numInOneBlock; j < nn_new_size && j < (k + 1) * numInOneBlock; j++)
                                {
                                    callback(nn_new[i], nn_new[j]);
                                }
                            }
                ...
                for (unsigned k = 0; k < block_old; k++)
                {
                    for (unsigned l = 0; l < block_new; l++)
                    {
                        for (unsigned i = l * numInOneBlock; i < nn_new_size && i < (l + 1) * numInOneBlock; i++)
                        {
                            for (unsigned j = k * numInOneBlock; j < nn_old_size && j < (k + 1) * numInOneBlock; j++)
                            {
...
#pragma omp parallel for default(shared) schedule(dynamic, 100) reduction(+ : cc)
            for (unsigned n = 0; n < oracle.size(); ++n)
            {
                ...
                nhoods[n].join2([&](unsigned i, unsigned j)
                                {
                    dist_type dist = oracle(i, j);
                    ++cc;
                    unsigned r;
...
// #pragma omp parallel for
        ...
        {
            string path;
            path = string("./result/bnlj_final/") + string("kgraph_data(") + data + string(")_L") + std::to_string(params.L) + string("_S") + std::to_string(params.S) + string("_R") + std::to_string(params.R) + string("_I") + std::to_string(params.iterations) + string("_K") + std::to_string(params.K) + string(".csv");
...
#pragma omp parallel for reduction(+ : hit_case)
    ...
    public:
            ...
            : oracle(o), params(p), pinfo(r), nhoods(o.size()), n_comps(0), truth_path(truth_path)
        {
            ...
            numInOneBlock = (L2_Cache_Size / 2) / (dim * sizeof(float) + params.L * sizeof(Neighbor)) / 2;
            cout << numInOneBlock << endl;
            ...
            for (unsigned it = 0; (params.iterations <= 0) || (it < params.iterations); ++it)
            {
                ...
                unsigned num = info.M / 2 + params.R;
                auto times1 = timer.elapsed();
                if (numInOneBlock >= num)
                {
                    join2();
                }
                else
