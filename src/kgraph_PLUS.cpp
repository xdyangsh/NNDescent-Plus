...
namespace kgraph
{
    ...
    struct Neighbor
    {
        uint32_t id;
        dist_type dist;
        char flag; // whether this entry is a newly found one
        Neighbor() {}
        Neighbor(unsigned i, dist_type d, char f = 0x0) : id(i), dist(d), flag(f)
        {
        }
    };
    ...
    */
    ...
    void LinearSearch(IndexOracle const &oracle, unsigned i, unsigned K, vector<Neighbor> *pnns)
    {
        ...
        Neighbor nn;
        nn.id = 0;
        nn.flag = 0x0; // we don't really use this
        unsigned k = 0;
        while (nn.id < N)
        {
            if (nn.id != i)
        ...
        */
        ...
        void prune2()
        {
            ...
            for (unsigned i = 0; i < graph.size(); ++i)
            {
                ...
                for (auto &e : graph[i])
                {
                    e.flag = 0x1; // haven't been visited yet
                }
            }
            ...
            for (unsigned l = 0; todo.size(); ++l)
            {
                ...
                for (unsigned i : todo)
                {
                    ...
                    v[l].flag = 0x0; // now we have seen this one
                    ++progress;
                    unsigned T;
                    {
                        auto &nl = new_L[i];
...
#pragma omp critical
    ...
    class KGraphConstructor : public KGraphImpl
    {
        ...
        { // neighborhood
            ...
            unsigned rnn_new_sum;
            unsigned rnn_old_sum;
            ...
            unsigned parallel_try_insert(unsigned id, dist_type dist)
            {
                ...
                unsigned l = UpdateKnnList(&pool[0], L, Neighbor(id, dist, 0x0));
                if (l <= L)
                { // inserted
                    if (L + 1 < pool.size())
            ...
            void join(C callback)
            {
                    ...
                    if (pool[i].flag == 0x2)
                    {
                        for (unsigned j = i + 1; j < L; j++)
                        {
                            ...
                            { //(nn_new) and (nn_new nn_old)
                                callback(pool[i].id, pool[j].id);
                            }
                        }
                        for (unsigned const l : rnn_new)
                        { //(nn_new) and (rnn_new)
                            callback(pool[i].id, l);
                        }
                        for (unsigned const l : rnn_old)
                        { //(nn_new) and (rnn_old)
                            callback(pool[i].id, l);
                        }
                        pool[i].flag = 0x1;
                ...
                for (unsigned const i : rnn_new)
                {
                    for (unsigned const j : rnn_new)
                    {
                        if (i < j)
                        {
                            callback(i, j);
            ...
            template <typename C>
            void join(C callback, unsigned numInOneBlock)
            {
                for (unsigned i = 0; i < L; i++)
                {
                    // std::cout<<pool[i].flag<<std::endl;
                ...
                unsigned rnn_new_size = rnn_new.size();
                unsigned rnn_old_size = rnn_old.size();
                unsigned block_new = ceil(rnn_new_size / float(numInOneBlock));
                unsigned block_old = ceil(rnn_old_size / float(numInOneBlock));
                for (unsigned k = 0; k < block_new; k++)
                {
                    for (unsigned l = k; l < block_new; l++)
                    {
                        if (k == l)
                        {
                            for (unsigned i = l * numInOneBlock; i < rnn_new_size && i < (l + 1) * numInOneBlock; i++)
                            {
                                for (unsigned j = i + 1; j < rnn_new_size && j < (k + 1) * numInOneBlock; j++)
                                {
                                    callback(rnn_new[i], rnn_new[j]);
                                }
                            }
                        }
                        else
                        {
                            ...
                            for (unsigned i = l * numInOneBlock; i < rnn_new_size && i < (l + 1) * numInOneBlock; i++)
                            {
                                for (unsigned j = k * numInOneBlock; j < rnn_new_size && j < (k + 1) * numInOneBlock; j++)
                                {
                                    callback(rnn_new[i], rnn_new[j]);
                                }
                            }
                ...
                for (unsigned k = 0; k < block_old; k++)
                {
                    for (unsigned l = 0; l < block_new; l++)
                    {
                        for (unsigned i = l * numInOneBlock; i < rnn_new_size && i < (l + 1) * numInOneBlock; i++)
                        {
                            for (unsigned j = k * numInOneBlock; j < rnn_old_size && j < (k + 1) * numInOneBlock; j++)
                            {
                                callback(rnn_new[i], rnn_old[j]);
                            }
                        }
                    }
                }
        ...
        void init()
        {
            ...
            for (auto &nhood : nhoods)
            {
                nhood.rnn_new.resize(params.S * 2);
            }
#pragma omp parallel
            {
#ifdef _OPENMP
...
#pragma omp for
                for (unsigned n = 0; n < N; ++n)
                {
                    ...
                    GenRandom(rng, &nhood.rnn_new[0], nhood.rnn_new.size(), N);
...
#pragma omp parallel for default(shared) schedule(dynamic, 100) reduction(+ : cc)
            for (unsigned n = 0; n < oracle.size(); ++n)
            {
                ...
                nhoods[n].join([&](unsigned i, unsigned j)
                    ...
                    if (r < params.K) ++uu; }, numInOneBlock);
                nhoods[n].found = uu > 0;
            }
            n_comps += cc;
        }
                ...
                for (unsigned l = 0; l < nhood.M; ++l)
                {
                    ...
                    if (nn.flag == 0x0)
                    {
                        ...
                        if (nn.dist > nhood_o.radiusM)
                        {
                            ...
                            nhood_o.rnn_new_sum++;
                            if (nhood_o.rnn_new_sum <= params.R)
                            {
                            ...
                            else
                            {
                                unsigned seed = params.seed;
                                mt19937 rng(seed);
                                unsigned rnn_new_index = rng() % nhood_o.rnn_new_sum;
                                if (rnn_new_index < params.R)
                                {
                                    nhood_o.rnn_new[rnn_new_index] = n;
                                }
                            }
                        }
                    }
                    else
                    {
                        ...
                        if (nn.dist > nhood_o.radiusM)
                        {
                            LockGuard guard(nhood_o.lock);
                            nhood_o.rnn_old_sum++;
                            if (nhood_o.rnn_old_sum <= params.R)
                            {
                            ...
                            else
                            {
                                unsigned seed = params.seed;
                                mt19937 rng(seed);
                                unsigned rnn_old_index = rng() % nhood_o.rnn_old_sum;
                                if (rnn_old_index < params.R)
                                {
                                    nhood_o.rnn_old[rnn_old_index] = n;
                                }
                            }
                        }
                    }
...
//             }
        ...
        {
            string path;
            path = string("./result/++/") + string("kgraph_data(") + data + string(")_L") + std::to_string(params.L) + string("_S") + std::to_string(params.S) + string("_R") + std::to_string(params.R) + string("_I") + std::to_string(params.iterations) + string("_K") + std::to_string(params.K) + string(".csv");
...
#pragma omp parallel for reduction(+ : hit_case)
    ...
    public:
            ...
            : oracle(o), params(p), pinfo(r), nhoods(o.size()), n_comps(0), truth_path(truth_path)
        {
            ...
            printf("L2_Cache_Size：%ld\n", L2_Cache_Size);
            unsigned dim = oracle.dim();
            numInOneBlock = (L2_Cache_Size / 2) / (dim * sizeof(short) + params.L * sizeof(Neighbor)) / 2;
            bool isjoin = true;
            if (numInOneBlock >= params.R)
                isjoin = false;
