        ...
        */
        ...
        virtual unsigned search (SearchOracle const &oracle, SearchParams const &params, unsigned *ids, dist_type *dists, SearchInfo *pinfo) const {
            ...
            for (unsigned trial = 0; trial < params.T; ++trial) {
                ...
                while (k < L) {
                    auto &e = knn[k];
                    if (e.flag==0x1) { // all neighbors of this node checked
                        ++k;
                        continue;
                    }
                    unsigned beginM = e.m;
        ...
        void prune2 () {
            ...
            for (unsigned l = 0; todo.size(); ++l) {
                ...
                for (unsigned i: todo) {
                    ...
                    if (v[l].flag==0x0) continue; // we have visited this one already
...
#pragma omp parallel for 
        ...
        {
            string path;
            path=string("./result/++/")+string("kgraph_data(")+data+string(")_L")+std::to_string(params.L)
...
public:
            ...
            : oracle(o), params(p), pinfo(r), nhoods(o.size()), n_comps(0),truth_path(truth_path)
        {
            ...
            bool isjoin=true;
            if(numInOneBlock>=params.R) isjoin=false;
